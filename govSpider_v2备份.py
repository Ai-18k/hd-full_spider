...
backtrace_ = True
diagnose_ = True
format_ = '<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: ' \
          '<8}</level> ' \
          '| <magenta>{process}</magenta>:<yellow>{thread}</yellow> ' \
          '| <cyan>{name}</cyan>:<cyan>{function}</cyan>:<yellow>{line}</yellow> - ' \
          '<level>{message}</level>'
# info
logger.add(folder_ + prefix_ + "info.log",
           level="INFO",
...
# 线程级代理分配表，确保同一线程重用同一代理，减少验证请求
_THREAD_PROXY_MAP: Dict[int, Dict[str, str]] = {}
_THREAD_PROXY_LOCK = threading.Lock()
...
class Govspider(JY):
    ...
    def __init__(self):
        ...
        self.CURRENT_ACCOUNT_KEY = "gov:01";
        self.backcomp = []
    ...
    def loginuser(self, user, params):
        ...
        if r.status_code in (200, 302):
            ...
            if ir.status_code == 200: self.iv8_env(ir.text); logger.success("首页cookie已更新")
            return True
    ...
    def searchcompany(self, company, page):
        while True:
            try:
                ...
                r = self.unified_request(url=url, method="POST", data=data, timeout=(5, 15), custom_cookie=self.cookie,
                                         retry_func=rt)
                if r.status_code == 200:
